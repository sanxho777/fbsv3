(() => {
  ...
  /* fallback: look inside Ã¢â‚¬Å“Vehicle PreviewÃ¢â‚¬Â side box (Cars.com) */
  ...
  function mountOverlay(){
      ...
      <div class="vp-modal" id="vp-modal">
        <div class="vp-hdr">
          <div>
            ...
            <div class="vp-sub">source: <span id="vp-source">Ã¢â‚¬â€œ</span></div>
          </div>
    ...
    let last = null;
    function render(v){
      srcEl.textContent = v.source || "Ã¢â‚¬â€œ";
      const entries = [
    ...
    scan.addEventListener("click", () => {
      ...
      render(last);
      msg.textContent = "Scanned Ã¢Å“â€œ";
    });
    // ---- UNIVERSAL SEND: include all aliases + nested wrappers so any bridge mapping works ----
    send.addEventListener("click", async () => {
      try {
        ...
        const ext = pick(last.exterior, last.exteriorColor, last.Exterior, last.extColor, last.color_exterior);
        const intr = pick(last.interior, last.interiorColor, last.Interior, last.intColor, last.color_interior);

        const alias = {
          exterior: ext, Exterior: ext, exteriorColor: ext, extColor: ext, color_exterior: ext,
          interior: intr, Interior: intr, interiorColor: intr, intColor: intr, color_interior: intr
        };
        ...
        const payload = { ...last, ...alias, ...wrappers };

        // Little console breadcrumb so you can confirm locally
        try { console.log("[VP] posting to bridge:", payload); } catch {}
        ...
        msg.textContent = res.ok ? "✅ Sent to Node bridge." : `⚠️ Bridge error ${res.status}`;
      } catch (e) {
    ...
    send.addEventListener("click", async () => {
      try{
        if (!last) { last = detectAndCapture(); render(last); }
        msg.textContent = "SendingÃ¢â‚¬Â¦";
        const res = await fetch("http://localhost:3323/capture", {
          method:"POST", headers:{ "Content-Type":"application/json" },
          body: JSON.stringify(last)
        });
        msg.textContent = res.ok ? "Ã¢Å“â€¦ Sent to Node bridge." : `Ã¢Å¡Â Ã¯Â¸Â Bridge error ${res.status}`;
      }catch(e){ msg.textContent = "Ã¢Å¡Â Ã¯Â¸Â " + (e?.message||e) }
    });
