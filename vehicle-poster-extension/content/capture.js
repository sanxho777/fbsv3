...
/* === VP inject: expose __captureMileage/__capturePrice to PAGE (not just content world) === */
(function(){
  if (window.__vpPageInjectApplied) return; window.__vpPageInjectApplied = true;
  const code = `(function(){
    try{
      if (window.__vpHeaderFnsInstalled) return; window.__vpHeaderFnsInstalled = true;
      function __vp_price(){
        try{
          const nodes = Array.from(document.querySelectorAll("h1,h2,h3,[class*='price'],[data-test],body *")).slice(0,2000);
          for(const el of nodes){ const t=(el.textContent||"").replace(/\\s+/g," ").trim();
            const m=t.match(/\\$\\s?(\\d{1,3}(?:,\\d{3})+)/); if(m) return Number(m[1].replace(/,/g,"")); }
        }catch{} return null;
      }
      function __vp_miles(){
        try{
          const nodes = Array.from(document.querySelectorAll("h1,h2,h3,[class*='mileage'],[data-test],body *")).slice(0,2000);
          for(const el of nodes){ const t=(el.textContent||"").replace(/\\s+/g," ").trim();
            const m=t.match(/(\\d{1,3}(?:,\\d{3})+|\\d{2,6})\\s*(?:mi\\.?|miles?)/i); if(m) return Number(m[1].replace(/,/g,"")); }
        }catch{} return null;
      }
      window.__capturePrice   = __vp_price;
      window.__captureMileage = __vp_miles;
      console.debug("[VP] helpers ready in page");
    }catch(e){console.debug("[VP] inject error:", e&&e.message||e)}
  })();`;
